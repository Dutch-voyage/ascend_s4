...
def _build_args(y_in__, y_out_, num_rows, num_columns, batch_shape, dtype):
    __inputs__ = []
    for arg in [y_in__]:
        if arg != None:
    ...
    if num_columns != None:
        attr = {}
        attr["name"] = "num_columns"
        attr["dtype"] = "int"
        attr["value"] = num_columns
        __attrs__.append(attr)
    if batch_shape != None:
        attr = {}
        attr["name"] = "batch_shape"
        attr["dtype"] = "list_int"
        attr["value"] = batch_shape
        __attrs__.append(attr)
    if dtype != None:
        attr = {}
        attr["name"] = "dtype"
...
@tbe_register.register_operator("EyeCustom", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def eye_custom(y_in__, y_out_, num_rows, num_columns=0, batch_shape=[], dtype=0, kernel_name="eye_custom", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, y_out_, num_rows, num_columns, batch_shape, dtype)
    ...
    custom_all_compile_options = {'__ALLSOC__': ['-fopenmp']},
    ...
    origin_func_name = "eye_custom"
    ascendc_src_dir_ex = "eye_custom"
    ascendc_src_dir = "eye_custom"
    ascendc_src_file = "eye_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator EyeCustom, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "EyeCustom"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[y_in__], origin_outputs = [y_out_],\
...
def op_select_format(y_in__, y_out_, num_rows, num_columns=0, batch_shape=[], dtype=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, y_out_, num_rows, num_columns, batch_shape, dtype)
    result = check_op_cap("op_select_format", "EyeCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(y_in__, y_out_, num_rows, num_columns=0, batch_shape=[], dtype=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, y_out_, num_rows, num_columns, batch_shape, dtype)
    result = check_op_cap("get_op_specific_info", "EyeCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
