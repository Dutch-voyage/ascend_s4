...
[[maybe_unused]] uint32_t NNOPBASE_EyeCustom = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnEyeCustomTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "EyeCustom";
    char inputDesc[] = {1};
    char outputDesc[] = {1};
    char attrDesc[] = {1, 0, 0, 0};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(yRef);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, yRef, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&numRows), sizeof(int64_t), 0, kNnopbaseInt));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&numColumns), sizeof(int64_t), 1, kNnopbaseInt));
    if (batchShapeOptional) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddIntArrayAttr(*executor, batchShapeOptional, 2));
    } else {
        static int64_t batchShapeOptionalDef[] = {};
        static size_t batchShapeOptionalLen = 1;
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddArrayAttrWithDtype(*executor, static_cast<void*>(batchShapeOptionalDef), batchShapeOptionalLen, sizeof(int64_t), 2, kNnopbaseInt));
    }
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&dtype), sizeof(int64_t), 3, kNnopbaseInt));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddOutput(*executor, yRef, 0));
    if (NnopbaseAddParamName != NULL) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 0, "yRef", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 0, "yRef", false));
    }
